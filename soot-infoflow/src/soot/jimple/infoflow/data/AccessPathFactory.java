...
import soot.jimple.infoflow.util.CopyOnWriteTCustomHashSet;
...
public class AccessPathFactory {
	...
	private MyConcurrentHashMap<Type, CopyOnWriteTCustomHashSet<AccessPathFragment[]>> baseRegister = new MyConcurrentHashMap<>();
	...
	private void registerBase(Type eiType, AccessPathFragment[] base) {
		Set<AccessPathFragment[]> bases = baseRegister.computeIfAbsent(eiType,
				t -> new CopyOnWriteTCustomHashSet<>(new HashingStrategy<AccessPathFragment[]>() {
	...
	public CopyOnWriteTCustomHashSet<AccessPathFragment[]> getBaseForType(Type tp) {
