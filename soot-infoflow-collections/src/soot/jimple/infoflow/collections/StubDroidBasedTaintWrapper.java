    ...
    protected AccessPath createAccessPathFromTaint(Taint t, Stmt stmt, AccessPathPropagator curr, boolean reverseFlows) {
        ...
        if (t.isReturn()) {
            ...
            return manager.getAccessPathFactory().createAccessPath(defStmt.getLeftOp(), baseType, fragments,
                    t.taintSubFields(), false, true, AccessPath.ArrayTaintType.ContentsAndLength);
        }
        ...
        if (t.isReturn()) {
            ...
            for (Unit unit : sm.getActiveBody().getUnits()) {
                ...
                AccessPath ap = manager.getAccessPathFactory().createAccessPath(((ReturnStmt) unit).getOp(), baseType,
                        fragments, t.taintSubFields(), false, true, AccessPath.ArrayTaintType.ContentsAndLength);
                aps.add(ap);
            }
            return aps;
        }
